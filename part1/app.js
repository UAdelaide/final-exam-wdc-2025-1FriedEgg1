...
app.get('api/walkers/summary', async (req, res) => {
  try {
    const [rows] = await db.query('SELECT u.username AS walker_username, COUNT(wr.request_id) AS completed_walks, AVG(wr.rating) AS average_rating, COUNT(wr.rating) AS total_ratings FROM');
    return res.json(rows);
  } catch (err) {
    console.log(err);
  }
